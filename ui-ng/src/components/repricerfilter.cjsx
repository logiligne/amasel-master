...
RepricerFilter = React.createClass
	mixins: [FormMixin]
	getInitialState: ()->
		return {
			config: [
	...
	render: ()->
		<WrapFilter {...@props} config={@state.config} getValuesMethod={@getValues}>
			<Row>
				...
				<Col xs={2}>
					<Input type='radio' label='Active only' name="active"/>
				</Col>
				<Col xs={2}>
					<Input type='radio' label='Inactive only' name="active"/>
				</Col>
			</Row>
		</WrapFilter>

module.exports = RepricerFilter
