(function() {
  var AmazonTransaction, DbModel, INSTANCES, IndexedCollection, PouchDB, Selection, _;
  ...
  DbModel = (function() {
    ...
    DbModel.prototype._clear = function() {
      this.unshippedOrders = new IndexedCollection({
        primaryIndex: "AmazonOrderId",
        autoGroups: {
          "flags.*": function(value, key) {
            return value != null ? value.flags : void 0;
          }
        }
      });
      this.products = new IndexedCollection({
        primaryIndex: "ASIN",
        columnIndexes: ["SKU"]
      });
      this.purchaseOrders = new IndexedCollection({
        primaryIndex: "SKU"
      });
      this.billingReports = [];
      this.repricerHistory = new IndexedCollection({
        primaryIndex: "time"
      });
      this.productSelection = new Selection();
      this.productPriceChanges = new AmazonTransaction();
      this.productQuantityChanges = new AmazonTransaction();
      return this.productSelection.refEvents('_selection_link').on('changed', (function(_this) {
        return function(key, value) {
          if (value) {
            return _this.products.addToGroup('selection', _this.products.getByIndex('SKU', key).ASIN);
          } else {
            return _this.products.removeFromGroup('selection', _this.products.getByIndex('SKU', key).ASIN);
          }
        };
      })(this));
    };
    ...
    DbModel.prototype._processResult = function(dataList) {
      var ASIN, data, doc, flags, i, j, len, len1, ref, ref1, row, v;
        ...
        for (j = 0, len1 = ref.length; j < len1; j++) {
          ...
          switch (doc.objType) {
            ...
            case "itemInfo":
              ...
              break;
            case "repricerConfig":
              ASIN = (ref1 = this.products.getByIndex('SKU', doc.SKU)) != null ? ref1.ASIN : void 0;
              if (ASIN) {
                this.products.update(ASIN, {
                  repricerConfig: doc.config
                });
              }
              break;
            case "order":
    ...
    DbModel.prototype.fetchUnshippedOrders = function(options) {
      ...
      return this._fetchView('unshipped_orders', options).then((function(_this) {
        return function(result) {
          var keys;
    ...
    DbModel.prototype.fetchRepricerConfig = function(options) {
      if (options == null) {
        options = {};
      }
      return this._fetchView('repricer_config', options).then((function(_this) {
        return function(result) {
          return _this;
        };
      })(this));
  ...
  module.exports = {
    ...
    get: function(name, config) {
      ...
      return INSTANCES[name] = new DbModel(config);
    }
  };
